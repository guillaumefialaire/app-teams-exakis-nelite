...
export default class ProjectForm extends React.Component<ProjectFormProps, ProjectFormState>{
    ...
    public async componentDidMount() {
        ...
        if (id !== undefined && id !== null) {    
            let project = await projectService.loadById(id);
            this.setState({
                name : project.title
            });
            if(project.description){
                if(project.description.englishStr)
                this.setState({enDesc : (project.description.englishStr) })
                if(project.description.frenchStr)
                this.setState({frDesc : (project.description.frenchStr) })
            }
            if(project.businessSector){
                this.setState({
                    businessSectorInit : [{
                        key : project.businessSector.ID.toString(),
                        name : project.businessSector.Label,
                        path : undefined,
                        termSet : undefined
                    }]
                });
            ...
            if(project.customerID){
                this.setState({
                    ...
                    defaultCustomer : [{key : project.customerID, name: project.customerName}],
                    outsideCustomerHidden : true                })
            } 
            if(project.outsideCustomerName){
                this.setState({itemPickerDisabled : true,
                    labelCustomerClass : styles.textDisabled,
                    outsideCustomerName : project.outsideCustomerName,
                defaultCustomer : [],
    ...
    public async save(){
        ...
        let id = await projectService.save(project);
